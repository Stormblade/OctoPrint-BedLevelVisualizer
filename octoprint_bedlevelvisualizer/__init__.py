...
class bedlevelvisualizer(octoprint.plugin.StartupPlugin,
    ...
    def processGCODE(self, comm, line, *args, **kwargs):
        ...
        if self._settings.get_boolean(["ignore_correction_matrix"]) and re.match(r"^(Mesh )?Bed Level (Correction Matrix|data):.*$", line.strip()):
            line = "ok"
        if self.processing and "ok" not in line and re.match(r"^((Z\-probe.+)|(G33.+)|(Bed.+)|(\d+\s)|(\|\s*)|(\s*\[\s+)|(\[?\s?\+?\-?\d?\.\d+\]?\s*\,?)|(\s?\.\s*)|(NAN\,?))+(\s+\],?)?$", line.strip()):
            ...
            if re.match(r"^Z\-probe:.+$", line.strip()):
                self.repetier_firmware = True
                self._bedlevelvisualizer_logger.debug("using repetier Z-Probe flag")
        ...
        if self.processing and ("ok" in line or (self.repetier_firmware and "Transformation matrix" in line)) and len(self.mesh) > 0:
            ...
            self._bedlevelvisualizer_logger.debug("STEP B")
            ...
            if self.old_marlin or self.repetier_firmware:
                ...
                x = np.unique(a[1]).astype(np.float)
                y = np.unique(a[2]).astype(np.float)
                z = a[0].reshape((len(x),len(y)))
